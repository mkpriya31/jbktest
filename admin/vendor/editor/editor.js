			...
			var menuItems = { 'fonteffects': true,
						...
						'insert_table'	: { "modal": true,
						   ...
						   'outdent'	: { "text": "Outdent", 
							...
							'source'	: { "modal": true,
						   					"modalId": "InsertCode_" + $(this).attr("id"),
											"icon":"fa fa-code", 
											"tooltip": "Insert Code", 
											"modalHeader": "Insert Code",
											"modalBody": $('<div/>',{   class:"form-group"
																	}).append($('<div/>',{
																		id :"errMsg_" + $(this).attr("id")
																	})).append($('<textarea/>',{
																		id:"inputCode_" + $(this).attr("id"),
																		class:"form-control form-control-code ",
																		placeholder:"Text to Display",
																		//html: methods.getText.apply(),
																		html: $('.scode').html(),
																		style: 'height: 250px'
																	})),
											"beforeLoad":function(){ 
												...
												$(".alert").alert("close");
												if($(editorObj).data('currentRange')!=''){ 												
													$('#inputCode_' +  _idSuffix).val($(editorObj).data('currentRange'));										
												}
												//$(this).data("editor").children().first().text();
											},
											"onSave":function(){
												var _idSuffix = "_" + editorObj.attr("id");
												var targetCode = $('#inputCode' + _idSuffix).val();
												$('.scode').empty('');
												if(targetCode ==''){
													methods.showMessage.apply(editorObj,["errMsg","Please enter code"]);
													return false;
												}																						
												else{
													
													if($(".scode").length>=1){
														var targetSourceCode=targetCode;
														
													}
													else{
														var targetSourceCode='<pre><code class="scode">'+targetCode+'</code></pre>';
													}
													
													methods.restoreSelection.apply(editorObj,[targetSourceCode,'html']);
												}
												
												//$(editorObj).data("editor").find('a[href="'+targetURL+'"]').each(function(){ $(this).attr("target", "_blank"); });
												$(".alert").alert("close");
												$("#InsertCode" + _idSuffix).modal("hide");
												$(editorObj).data("editor").focus();
